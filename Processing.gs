...
function idxOf(needle,haystack) {
  for(var i in haystack){
    if(Array.isArray(haystack[i])){
      for(var x in haystack[i]){
        if(haystack[i][x].indexOf(needle) != -1){
          return i;
        }
      }
    }
    else if(haystack[i].indexOf(needle) != -1){
      return i;
    }
  }
  return -1;
...
function toProperCase(txt){
  var idx=0;
  var regexA = hackRegex(/\p{L}\p{M}*/);
  while(txt.charAt(idx).match(regexA)===false){
    if(idx==txt.length-1){ break; }
    idx++;
  }
  var regexB = hackRegex(/\p{L&}\p{M}*/);
  if(txt.charAt(idx).match(regexB)){
    if(idx<txt.length-2){ return txt.substr(0,idx) + txt.charAt(idx).toUpperCase() + txt.substr(idx+1).toLowerCase(); }
    else{ return txt; }      
  }
  else{ return txt; }
  return txt;
...
function processQueries(queries,versions){
  ...
  for(var l in versions){
    indexes[versions[l]] = JSON.parse(scriptProperties.getProperty(versions[l]+"IDX"));
  }
  //docLog(JSON.stringify(indexes));
  for(var x in queries){
    var thisquery = toProperCase(queries[x]);
    if(x===0){
    ...
    var temp = checkQuery(thisquery,indexes,thisbook);
    if(temp !== false){
      //docLog("thisbook as returned from checkQuery = "+thisbook);
      goodqueries.push(thisquery);          
    }else{
      thisbook = temp;
    }
  }
...
function checkQuery(thisquery,indexes,thisbook){
  ...
  var match;
  if(match = thisquery.match(regexB)){
    //docLog('we have a match for regexB');
    thisbook = match[0];
    //docLog("We have gotten the value of thisbook in checkQuery: "+thisbook);
    ...
    if(validBookIndex != -1){
      //we can remove book from query now
      ...
      if(thisquery.indexOf('.') != -1){
        ...
        var m;        
        while ((m = re.exec(thisquery)) != null) {
      ...
      if(thisquery.indexOf(',') != -1){
        if(thisquery.match(/[1-9][0-9]{0,2}\,[1-9][0-9]{0,2}/g) === null || thisquery.match(/[1-9][0-9]{0,2}\,[1-9][0-9]{0,2}/g).length != thisquery.match(/\,/g).length){ 
          ...
          var regex = /([1-9][0-9]{0,2})\,/g;
          var match;
          var matches = [];
          while(match = regex.exec(thisquery)){
            matches.push(match[1]); //0=whole match,1=first capturing group
          }
          //docLog(JSON.stringify(matches));
          var myidx = validBookIndex+1;
          for(var k in matches){ //for each chapter captured in the querystring
            for(var j in indexes){ //for each version in the indexes
              //docLog(JSON.stringify(indexes[j]["book_num"]));
              //docLog(myidx);
              var bookidx = indexes[j]["book_num"].indexOf(myidx);
              //docLog(bookidx);
              var chapter_limit = indexes[j]["chapter_limit"][bookidx];
              if(matches[k]>chapter_limit){
                msg = _r('A chapter in the query is out of bounds: there is no chapter <{0}> in <{1}> in the requested version <{2}>, the last possible chapter is {3}',locale);
                alertMe(msg.format(matches[k],thisbook,j,chapter_limit));
                return false;
              }
            }
          }
          
          var commacount = thisquery.match(/,/g).length;
          if(commacount>1){
            if(thisquery.indexOf('-')==-1){ alertMe(_r('You cannot have more than one colon and not have a dash!',locale)+' <'+thisquery+'>'); return false; }
            var parts = thisquery.split('-');
            if(parts.length != 2){ alertMe(_r('You seem to have a malformed querystring, there should be only one dash.',locale)+' <'+thisquery+'>'); return false; }
            for(var p in parts){
              var pp = parts[p].split(',').map(parseInt); //pp[0]=chapter, pp[1]=verse
              for(var j in indexes){ //for each version in the indexes
                //docLog(JSON.stringify(indexes[j]["book_num"]));
                //docLog(myidx);
                var bookidx = indexes[j]["book_num"].indexOf(myidx);
                //docLog(bookidx);
                var chapters_verselimit = indexes[j]["verse_limit"][bookidx];
                var verselimit = parseInt(chapters_verselimit[pp[0]-1]);
                if(pp[1]>verselimit){
                  msg = _r('A verse in the query is out of bounds: there is no verse <{0}> in <{1}> chapter <{2}> in the requested version <{3}>, the last possible verse is {4}',locale); 
                  alertMe(msg.format(pp[1],thisbook,pp[0],j,verselimit));
                  return false;
                }
              }              
            }
            ...
            var parts = thisquery.split(',');//parts[0]=chapter
            if(parts[1].indexOf('-')!=-1){
              //docLog('we have a dash in '+thisquery);
              if(thisquery.match(/[,\.][1-9][0-9]{0,2}\-([1-9][0-9]{0,2})/)!==null){
                //docLog('we have a dash which is preceded by a comma or dot construct');
                var match;
                var matches = [];
                var regex = /[,\.][1-9][0-9]{0,2}\-([1-9][0-9]{0,2})/g;
                while(match = regex.exec(thisquery)){
                  matches.push(match[1]);
                }
                var highverse = parseInt(matches.pop());
                //docLog(highverse);
                for(var j in indexes){ //for each version in the indexes
                  //docLog(JSON.stringify(indexes[j]["book_num"]));
                  //docLog(myidx);
                  ...
                  //docLog(bookidx);
                  var chapters_verselimit = indexes[j]["verse_limit"][bookidx];
                  var verselimit = parseInt(chapters_verselimit[parseInt(parts[0])-1]);
                  if(highverse>verselimit){
                    msg = _r('A verse in the query is out of bounds: there is no verse <{0}> in <{1}> chapter <{2}> in the requested version <{3}>, the last possible verse is {4}',locale);
                    alertMe(msg.format(highverse,thisbook,parts[0],j,verselimit));
                    return false;
                  }
                }                
              }
              ...
              var match = thisquery.match(regex);
              if(match!==null){
                var highverse = parseInt(match[1]);
            ...
            if(matches.length>0){
              ...
              for(var j in indexes){
                ...
                if(highverse>verselimit){
                  ...
                  alertMe(msg.format(highverse,thisbook,parts[0],j,verselimit));
                  return false;
                } //end if from line 357
              } //end for from line 353
            } //end if from line 351
          } //end elseif from line 300
        } //end else from line 253
      }else{ //end if from line 248 
        var chapters = thisquery.split('-'); //if there is no dash, this will result in a single chapter; otherwise in multiple chapters
        for(var z in chapters){
          for(var j in indexes){ //for each version in the indexes
            var bookidx = indexes[j]["book_num"].indexOf(validBookIndex+1);
            var chapter_limit = indexes[j]["chapter_limit"][bookidx];
            if(parseInt(chapters[z])>chapter_limit){
              msg = _r('A chapter in the query is out of bounds: there is no chapter <{0}> in <{1}> in the requested version <{2}>, the last possible chapter is {3}',locale);
              alertMe(msg.format(chapters[z],thisbook,j,chapter_limit));
              return false;
            }
          }          
        }
      } // end else from line 366
      ...
      //docLog('Uh oh, book was not recognized as valid.');
      alertMe(_r("The query does not have a valid book or book abbreviation: ",locale)+'<'+thisquery+'>');  
      return false;
    }                
  }
  else{
    //docLog("could not recognize book!");
    if(thisquery.match(/^[1-9][0-9]{0,2}/) === null){
      alertMe(_r("A query that doesn't start with a book indicator must however start with a valid chapter indicator!",locale)+'<'+thisquery+'>');
      return false;
    }
  }
  return thisbook;
...
function setScriptProps(){
  ...
  if(metadata !== false){
    if(metadata.hasOwnProperty("results")){
      ...
      //docLog('Received biblebooks data from server: '+biblebooks);
...
function testProps(){
  ...
  //if(userProperties.getProperty("biblebooks0")===null){}
  var usrProps = userProperties.getProperties();
  if(Object.keys(usrProps).length>0){
    for(var i in usrProps){
      //docLog(i+': '+usrProps[i]);
    }
  }else{
    //docLog("There isn't a single property set here.");
  }
}
*/
